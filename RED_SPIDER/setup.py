# multitool_redspider.py
# -*- coding: utf-8 -*-
"""
Multitool "RedSpider" — logo rouge, texte blanc/rouge, fond noir
"""
import os
import sys
import subprocess
...
MENU = [
    {"name": "PC-Info", "items": [
        {"file": "VMX105.bat", "title": "Computer Infos"},
        {"file": "ssesehs", "title": "Discord Notes"},
    ]},
    {"name": "Osint", "items": [
    ...
    {"name": "Discord", "items": [
        ...
        {"file": "Bot-Invite-server.py", "title": "Discord Invite Bod To Id"},
...
SUB_MENU = [
    {"name": "Virus Builder", "items": [
        {"file": "virus-builder.py", "title": "Virus Builder Tool"},
        {"file": "tool_a2.py", "title": "Tool A: Script Two"},
    ]},
    {"name": "Extra Tools B", "items": [
        {"file": "tool_b1.py", "title": "Tool B: Script Alpha"},
        {"file": "tool_b2.py", "title": "Tool B: Script Beta"},
        {"file": "tool_b3.spy", "title": "Tool B: Script Gamma"},
    ]},
    {"name": "Diagnostics", "items": [
        {"file": "diag_test.bat", "title": "System Check"},
    ]},
]
...
def build_categories_panel(categories, menu_key):
    ...
    if menu_key == "main":
        nav_inst += f" {rgb(*RED_RGB)}n{RESET}-{rgb(*WHITE_RGB)}Genre 2{RESET}"
    elif menu_key == "genre2":
        nav_inst += f" {rgb(*RED_RGB)}k{RESET}-{rgb(*WHITE_RGB)}Main Menu{RESET}"
    
    lines = [
        "",
        f"{BOLD}{rgb(*WHITE_RGB)}https://github.com/IxilaA1/RedSpider-Tools {RESET}",
        "",
...
def main():
    ...
    while True:
        try:
            ...
            # Correction des couleurs: Utilisation de RED_RGB et WHITE_RGB
            shell_line = f"{rgb(*RED_RGB)}({RESET}administrator{rgb(*RED_RGB)}@{rgb(*WHITE_RGB)}RedSpider{rgb(*RED_RGB)}){RESET}-{rgb(*RED_RGB)}[{RESET}{menu_indicator}{rgb(*RED_RGB)}]{RESET}"
