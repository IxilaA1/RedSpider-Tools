...
MENU = [
    {"name": "PC-Info", "items": [
        ...
        {"file": "discord_notes.txt", "title": "Discord Notes"},
    ]},
    {"name": "Osint", "items": [
...
def build_categories_panel(categories):
    cat_strip = make_category_strip(categories)
    lines = [
        "",
        f"{BOLD}{rgb(*WHITE_RGB)}https://github.com/IxilaA1/RedSpider-Tools {RESET}",  #  Nouvelle ligne ici
        "",
        f"{BOLD}{rgb(*WHITE_RGB)}   Choose a category {rgb(*RED_RGB)}({rgb(*WHITE_RGB)}Ctrl+C exit{rgb(*RED_RGB)}){RESET}",
        ""
    ]
    lines += cat_strip + ["", ""]
    return lines
...
def display_categories_screen(categories):
    os.system("cls" if os.name == "nt" else "clear")
    out = get_centered_display_content(build_categories_panel(categories))
...
def main():
    ensure_files_dir()
    shell_line = f"{rgb(*RED_RGB)}({RESET}administrator{rgb(*RED_RGB)}@{rgb(*WHITE_RGB)}RedSpider{rgb(*RED_RGB)}){RESET}-{rgb(*RED_RGB)}[{RESET}Basic{rgb(*RED_RGB)}]{RESET}"
    prompt = f"{rgb(*RED_RGB)}>> ${RESET} "
    cats = MENU
    try:
        while True:
            display_categories_screen(cats)
            print(shell_line)
            ...
            if ch.isdigit():
                ...
                if 0 <= i < len(cats):
                    ...
                    while True:
                        ...
                        display_category_items_screen(cat, cat_i, len(cats), disp)
                        print(shell_line)
                        sub = input(prompt).strip().lower()
                        if sub in ("b", "back"): break
                        if sub in ("exit", "quit"): return
                        if sub in ("n", "next"): cat_i = (cat_i+1)%len(cats); continue
                        if sub in ("p", "prev"): cat_i = (cat_i-1)%len(cats); continue
                        if sub.isdigit():
                            s = int(sub)-1
                            if 0 <= s < len(disp): run_in_cmd_for_item(disp[s])
                        else: input(f"{rgb(*RED_RGB)}Press Enter...{RESET}")
            else:
                input(f"{rgb(*RED_RGB)}Press Enter...{RESET}")
