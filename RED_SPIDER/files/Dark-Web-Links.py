...
# Les liens fournis sont des services cachés Tor (adresses .onion)
links = {
    "Search Engine": {
...
def format_links(links):
    """
    Formate la structure des liens en utilisant des caractères arborescents.
    """
    display_link = ""
    
    # Remplacer les appels de fonctions de couleur par des chaînes de caractères brutes
    def CategoryColor(text):
        return f"{text}" # Simuler une couleur pour la catégorie
    def SiteColor(text):
        return text # Pas de couleur pour le site
    def URLColor(text):
        return text # Pas de couleur pour l'URL

    for category, sites in links.items():
        # Utiliser un séparateur pour les catégories
        display_link += "\n" + "=" * 20 + "\n" + CategoryColor(category) + "\n"
        
        def add_sites(prefix, sites_dict, is_last_in_parent=False):
            nonlocal display_link
            items = list(sites_dict.items())
            
            for i, (site, url) in enumerate(items):
                is_last = (i == len(items) - 1)
                
                if isinstance(url, dict):
                    # C'est un sous-ensemble (ex: ASAP Market)
                    current_line = f"{prefix}{'└─ ' if is_last else '├─ '}{SiteColor(site)}\n"
                    display_link += current_line
                    
                    # Déterminer le nouveau préfixe pour le niveau inférieur
                    new_prefix = prefix + ('   ' if is_last else '│   ')
                    add_sites(new_prefix, url, is_last)
                else:
                    # C'est un lien final
                    line_start = f"{prefix}{'└─ ' if is_last else '├─ '}"
                    display_link += f"{line_start}{SiteColor(site)}: {URLColor(url)}" + "\n"
        
        add_sites("", sites)
        
    return display_link

# Correction de l'appel Title
print("--- Dark Web Links ---")
print()

try:
    formatted_links = format_links(links)
    # Remplacer les fonctions Slow/tor_banner/MainColor/Continue/Reset par print()
    print(formatted_links)

except Exception as e:
    # Remplacer la fonction Error par print()
